...
def insertUser():
    ...
    if request.method == 'POST':
        ...
        for json_data in list_bulk_data.items():
            #key_list=list(json_data.keys())
            #value_list=list(json_data.values())
            orgName=""
            count=count+1
            regNo=json_data[1]['holder_id']
            
            name=json_data[1]['holder_name']
            course=json_data[1]['event_name']
            certificate_Id=json_data[1]['cert_no']
